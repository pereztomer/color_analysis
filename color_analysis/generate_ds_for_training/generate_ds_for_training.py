...
def equalize_classes(healthy_patient_id_to_photo_id_dict: dict, polyp_patient_id_to_photo_id_dict: dict):
    """
    equalizing healthy class and polyp class to have the same number of patients
    :param healthy_patient_id_to_photo_id_dict: a string address to folder of the feature vector file
    :param polyp_patient_id_to_photo_id_dict: feature vector file name
    :return: 2 dictionaries healthy and polyp
    """
    if len(healthy_patient_id_to_photo_id_dict) < len(polyp_patient_id_to_photo_id_dict):
        raise Exception('There are more patients with polyp than healthy patients')
    healthy_patient_ids = healthy_patient_id_to_photo_id_dict.keys()
    polyp_patient_ids = polyp_patient_id_to_photo_id_dict.keys()

    healthy_patient_ids = list(set(random.sample(healthy_patient_ids, len(polyp_patient_ids))))
    assert len(healthy_patient_ids) == len(polyp_patient_ids)

    values_to_remove = []
    for value in healthy_patient_id_to_photo_id_dict.keys():
        if value not in healthy_patient_ids:
            values_to_remove.append(value)

    for v_remove in values_to_remove:
        del healthy_patient_id_to_photo_id_dict[v_remove]
...
def split_data_set(folder_address: str, json_file_name: str, query_healthy: str, query_polyp: str,
    ...
    healthy_patient_id_to_photo_id_dict, polyp_patient_id_to_photo_id_dict = equalize_classes(
        healthy_patient_id_to_photo_id_dict, polyp_patient_id_to_photo_id_dict)
    ...
    with open(f'{folder_address}/{approved_query}/train_validation_{json_file_name}', "w") as outfile:
...
def main():
    ...
    for approved_val in approved_values_list:
        ...
        query_polyp = f'select id, patientid from gixammain.dbo.ImageStudyPatient where deleted = 0 and rejected = 0 ' \
                      f'and C_PolypATLG1 = 1  and [position] = 0 and ({approved_query}) and locked = 1 and branchid in (4,6,8)'
        ...
        for j_file in ds_json_files:
            ...
            try:
                ...
                print(e)
            try:
                test_json_file(folder_address, json_file_name, query_healthy, query_polyp, approved_query, True)
            except Exception as e:
